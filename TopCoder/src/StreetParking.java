public class StreetParking {
		...
		for(int i=0;i<street.length();i++){
			...
			if(token=='B'){
				if(i<2){
					...
					if(street.charAt(i-1)=='-'&&street.charAt(i-2)=='-'){
						if(i>=3&&street.charAt(i-3)=='S')count-=2;
						else count-=3;
					}
					else if(street.charAt(i-1)=='-'&&street.charAt(i-2)!='-'){
						if(street.charAt(i-2)=='S')count-=1;
						else count-=2; 
					}
				...
				if(i==0)count-=1;
					...
					if(i>=2&&street.charAt(i-1)=='-'&&street.charAt(i-2)=='S'){
						count-=1;
	...
	public static void main(String[] args) {
		 ...
		 */
		System.out.println(freeParks(street[3]));
		System.out.println(freeParks("S--B"));
