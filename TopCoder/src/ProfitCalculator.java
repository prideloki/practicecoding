...
public class ProfitCalculator {
	public static int percent(String[] items){
		...
		for(int i=0;i<items.length;i++){
			double cost=Double.parseDouble(items[i].substring(7));
			double price=Double.parseDouble(items[i].substring(0,6));
			
			totalCost+=cost;
			totalPrice+=price;
		}
		
		return (int)(((totalPrice-totalCost)/totalPrice)*100);
	}
}
